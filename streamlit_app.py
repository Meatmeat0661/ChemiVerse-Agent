...
def load_resources():
    ...
    if not mol_path.is_absolute():
        mol_path = Path(__file__).resolve().parent / mol_path
    if not rxn_path.is_absolute():
        rxn_path = Path(__file__).resolve().parent / rxn_path
    db = AstroChemDatabase(mol_path, rxn_path)
    agent = AstroChemAgent(db)
    nautilus = NautilusRunner(settings.nautilus)
    return settings, db, agent, nautilus, is_streamlit_cloud()
...
def sidebar_status(settings, db, nautilus: NautilusRunner) -> None:
...
def page_simulation(nautilus: NautilusRunner, settings) -> None:
    st.header("Westlake 化学演化 + 绘图")
    st.caption("依赖本机已安装的 westlake / torch 与 example_simulation 网络")

    default_species = st.session_state.get("plot_species", settings.nautilus.default_species)
    sim_dir = st.text_input("模拟目录（相对 tutorial_root）", value=settings.nautilus.default_sim_dir)
    ...
    st.info(
        "示例（含氮）: `N2,NH3,HCN,H2CO` · 默认（含氧有机）: `CO,CH3OH,CH3OCH3`",
        icon="💡",
    )
    ...
    with col_a:
        run_sim = st.button("运行模拟", type="primary")
    with col_b:
        plot_only = st.button("仅绘图（需已有 res.pickle）")
    with col_c:
        if st.button("预览命令"):
            st.code(" ".join(nautilus.build_command(sim_dir, use_evolution)), language="bash")
...
def show_plot_results(plot_data: dict, settings) -> None:
    ...
    for idx, img in enumerate(images):
        ...
        if not path.exists():
            ...
            path = out_root / plot_data.get("run_id", "") / img.get("filename", "")
        with cols[idx % len(cols)]:
            st.caption(img.get("label") or path.name)
            if path.exists():
...
def main() -> None:
    ...
    if on_cloud:
        st.caption("☁️ Streamlit Cloud · 公网分子/反应查询")
    else:
        st.markdown("分子物性 / 反应网络检索 · Westlake 丰度演化可视化")

    sidebar_status(settings, db, nautilus)
    if on_cloud:
        st.sidebar.info("云端版仅支持分子查询；演化模拟请在本地运行。")

    if on_cloud:
        page_query(agent, db, settings)
        with st.expander("Westlake 演化（云端不可用）"):
            st.warning(
                "Streamlit Cloud 无法运行本机 Westlake / Nautilus 模拟。"
                "请在电脑上使用 `streamlit run streamlit_app.py` 或云服务器部署完整版。"
            )
    else:
        tab1, tab2 = st.tabs(["🔬 分子查询", "📈 Westlake 演化"])
        with tab1:
            page_query(agent, db, settings)
        with tab2:
            page_simulation(nautilus, settings)
